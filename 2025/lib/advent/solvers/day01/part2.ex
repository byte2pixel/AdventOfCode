defmodule Advent.Solvers.Day01.Part2 do
  @moduledoc """
  ...
  def solve(input, step_callback \\ nil, speed_context \\ nil) do
    ...
    {_final_position, _final_degrees, cross_count} =
      Enum.reduce(rotations, {50, 180.0, 0}, fn rotation, {pos, degrees, count} ->
        ...
        # Count how many times the dial crosses 0
        crosses = count_crosses(pos, new_pos, rotation)
        new_count = count + crosses
  ...
  # Helper to count how many times the dial crosses 0 between pos and new_pos
  defp count_crosses(pos, _new_pos, {dir, distance}) do
    total = abs(distance)

    positions =
      case dir do
        "L" ->
          Enum.map(1..total, fn step ->
            p = rem(pos - step, 100)
            if p < 0, do: p + 100, else: p
          end)

        "R" ->
          Enum.map(1..total, fn step -> rem(pos + step, 100) end)
